...
def getCaptureRequest():
    ...
    while (True):
            ...
            if (file_mode == FILE_MODE_DUMP):
                ...
                with open(FILE_TO_SAVE,"rb") as image:
                    ...
                    if (current_chunk_id == 0):
                        transfer_content = data[0:CHUNK_SIZE]
                        # print(f"transfer content index 0: {transfer_content}")
                    elif (current_chunk_id > 0):
                        end = current_chunk_id * CHUNK_SIZE
                        if (end >= len(data)):
                            end = len(data)
                        transfer_content = data[(current_chunk_id - 1) * CHUNK_SIZE : end]
                        # print(f"transfer content : {transfer_content}")

                    if (current_chunk_id * CHUNK_SIZE <= len(data)):
                        # print(f"transfer content ep 2 : {transfer_content}")
                        reply_frame = kiss_protocol.KISSProtocol.wrap_image_chunk(current_file_id,current_chunk_id,transfer_content,VR_PID_GET_IMAGE_DOWNLOAD)
        ...
        if (ser.in_waiting):
            ...
            # print(buf)
            # reply = kiss_protocol.KISSProtocol.wrap_frame(PAYLOAD_VR,PAYLOAD_VR_PID_CAPTURE,b'')
            frame = kiss_protocol.KISSProtocol.unwrap_frame(buf)
            # print(frame)
            if (frame['type'] == 'generic'):
                if frame['payload_id'] == PAYLOAD_ID_VR:
                    # print("VR Payload ID Detected")
                    if frame['pid'] == VR_PID_GET_STATUS:
                        ...
                        reply = kiss_protocol.KISSProtocol.wrap_frame(PAYLOAD_ID_VR,PID_ACK,b'')
                        ser.write(reply)
