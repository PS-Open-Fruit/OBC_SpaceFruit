...
void SD_SendFile(char *filename) {
    ...
    if (f_open(&file, filename, FA_READ) == FR_OK) {
        ...
        uwCRCValue = HAL_CRC_Calculate(&hcrc, (uint32_t *)buffer, 0); // Dummy call/Reset might be needed or just start accumulating?
        // Actually HAL_CRC_Calculate resets and calculates. we need Accumulate for chunks.
        // But first chunk needs Calculate? Or usually Initialize handle resets it.
        // __HAL_CRC_DR_RESET(&hcrc); // Explicit reset
        // With HAL, HAL_CRC_Calculate resets DR. HAL_CRC_Accumulate adds to it.
        // So first chunk: Calculate. Subsequent: Accumulate.
        // OR better: Reset manually, then always Accumulate.
        ...
        while (1) {
            ...
            // Calculate CRC on raw data
            // HAL_CRC_Accumulate takes length in Input Data Format units (Bytes).
            // Input buffer is uint32_t* but treated as bytes because we set CRC_INPUTDATA_FORMAT_BYTES
            HAL_CRC_Accumulate(&hcrc, (uint32_t *)buffer, bytesRead);
        ...
        // Send Footer with CRC
        sprintf(msg, "CHECKSUM:0x%08lX\r\nEXPORT_COMPLETE\r\n", uwCRCValue);
        HAL_UART_Transmit(&hlpuart1, (uint8_t*)msg, strlen(msg), 1000);
